...
QueuesPlanner::QueuesPlanner(rn::vki::HandleSurfaceKHR &&surface, rn::vki::HandlePhysicalDevice &&physicalDevice, std::vector<vk::QueueFamilyProperties2> &&familyProperties) noexcept :
	surface{std::move(surface)},
	physicalDevice{std::move(physicalDevice)},
	familyProperties{std::move(familyProperties)}
{}
...
QueueFamilyIndex QueuesPlanner::reserveQueue(const std::vector<QueueTypeMask> &types, const TransferGranularity granularity) {
	for (QueueTypeMask type : types) {
		for (uint32_t i = 0; i < familyProperties.size(); i++) {
			if (reservedCount[i] >= familyProperties[i].queueFamilyProperties.queueCount) {
				continue;
			}
...
bool QueuesPlanner::testQueueProperties(const QueueTypeMask type, const TransferGranularity &granularity, uint32_t i) {
	if ((type & QueueType::Presentation)) {
		if ( ! RN_VKI_TRACE(physicalDevice->getSurfaceSupportKHR(i, surface.get(), physicalDevice.table()))) {
			return false;
		}
	}

	if ((type & QueueType::Transfer)) {
		if (familyProperties[i].queueFamilyProperties.minImageTransferGranularity.width > granularity.width) {
			return false;
		}

		if (familyProperties[i].queueFamilyProperties.minImageTransferGranularity.height > granularity.height) {
			return false;
		}

		if (familyProperties[i].queueFamilyProperties.minImageTransferGranularity.depth > granularity.depth) {
			return false;
		}
	}
	...
	if (flags) {
		if ((type & QueueType::AnyOther)) {
			if ((familyProperties[i].queueFamilyProperties.queueFlags & flags) != flags) {
				return false;
			}
		} else {
			if (familyProperties[i].queueFamilyProperties.queueFlags != flags) {
				return false;
			}
		}
	}
