	...
	while (start < memoryProperties.memoryTypeCount) {
		...
		for (uint32_t blockIdx = 0; blockIdx < blocks.size(); blockIdx++) {
			...
			if (allocation.memory) {
				ngn::log::debug("rn::vlk::memory::Pool::alloc([{}]) => allocated memory: {} bytes offset, {} leaf(s)", requirements.size, allocation.offset, 1u << (block.levels - block.levelFromIdx(allocation.leafIdx)));
				return Handle{
					allocation.memory,
					allocation.offset,
					allocation.flags,
		...
		try {
			...
			if (memoryTypeIndex + 1 < memoryProperties.memoryTypeCount) {
				...
				ngn::log::warn("rn::vlk::memory::Pool::alloc([{}]) => out of host memory for memory type {:x}", requirements.size, memoryTypeIndex);
			...
			if (memoryTypeIndex + 1 < memoryProperties.memoryTypeCount) {
				//  try another memory type
				ngn::log::warn("rn::vlk::memory::Pool::alloc([{}]) => out of device memory for memory type {:x}", requirements.size, memoryTypeIndex);
...
void Pool::free(const Handle &handle) {
	...
	if (handle.blockIdx >= blocks.size()) {
		ngn::log::error("rn::vlk::memory::Pool::free({}, {}, {}) => unable to free memory handle, block index out-of-range: {} => {}", rn::vlk::id(handle.memory), handle.blockIdx, handle.leafIdx, handle.blockIdx, blocks.size());
	} else {
		blocks[handle.blockIdx].free(handle.leafIdx);

		ngn::log::debug("rn::vlk::memory::Pool::free({}, {}, {}) => memory block freed", rn::vlk::id(handle.memory), handle.blockIdx, handle.leafIdx);
	}
}
...
Block & Pool::appendBlock(uint32_t memoryTypeIndex, vk::MemoryPropertyFlags flags, const vk::MemoryRequirements &requirements) {
	...
	if (size > baseMemorySize) {
		ngn::log::warn("rn::vlk::memory::Pool::appendBlock({:x}, {}, [{}]) => created bigger block than expected: {} bytes > {} bytes, leaf {} bytes [{:x}: {}]", memoryTypeIndex, vk::to_string(flags), requirements.size, size, baseMemorySize, leafSize);
	} else {
		ngn::log::debug("rn::vlk::memory::Pool::appendBlock({:x}, {}, [{}]) => created new block: {} bytes, leaf {} bytes", memoryTypeIndex, vk::to_string(flags), requirements.size, size, leafSize);
	}
