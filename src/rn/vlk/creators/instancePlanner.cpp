...
Stage::Stage(Flow flow, std::vector<std::string> &availableExtensions, std::vector<std::string> &availableLayers, bool &flag) :
	flow{flow},
...
Stage & Stage::includeExtensions(const std::vector<std::string> &extensions, std::function<Flow (const std::vector<std::string> &)> callback) {
	if (flow != Flow::OK) {
		return *this;
	}
...
Stage & Stage::includeLayers(const std::vector<std::string> &layers, std::function<Flow (const std::vector<std::string> &)> callback) {
	if (flow != Flow::OK) {
		return *this;
	}
...
Selector::Selector() {
	...
	std::vector<vk::LayerProperties> layerProperties = RN_VLK_TRACE(vk::enumerateInstanceLayerProperties());

	availableLayers = util::map(layerProperties, [] (auto &properties) {
		return std::string{properties.layerName};
	});

	// load available instance extensions
	std::vector<vk::ExtensionProperties> extensionProperties = RN_VLK_TRACE(vk::enumerateInstanceExtensionProperties());

	availableExtensions = util::map(extensionProperties, [] (auto &properties) {
		return std::string{properties.extensionName};
	});
}

Stage Selector::stage(bool &flag) {
	return Stage{flow, availableExtensions, availableLayers, flag};
}

void Selector::apply(const Stage &stage) {
	switch (stage.flow) {
		case Flow::FAIL_ALL:
			flow = Flow::FAIL_ALL;
			[[fallthrough]];
...
Selector::Result Selector::getResult(bool &flag) {
	flag = all;
	return result;
}

InstancePlannerResult InstancePlanner::selectExtensionsAndLayers(const DebugCallbackCreator &debugCallbackCreator, const SurfaceCreator &surfaceCreator) {
	InstancePlannerResult instancePlannerResult{};
	Selector selector{};

	// surface
	selector.apply(selector.stage(instancePlannerResult.surface)
		.includeExtensions(surfaceCreator.requiredExtensions, [] (const std::vector<std::string> &missingExts) {
	...
	selector.apply(selector.stage(instancePlannerResult.debugCallback)
		.includeExtensions(debugCallbackCreator.optionalExtensions, [] (const std::vector<std::string> &missingExts) {
			size_t size = missingExts.size();

			ngn::log::warn("Vulkan debug reporting disabled, missing {} extension(s): {}", size, util::implode(missingExts));
			return Flow::SKIP;
		})
		.includeLayers(debugCallbackCreator.optionalLayers, [] (const std::vector<std::string> &missingLayers) {
